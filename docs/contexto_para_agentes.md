...
### 6.5 Suite de Calidad — Pantalla 4

Corre pasadas automatizadas de **funcional / seguridad / accesibilidad** sobre aprendizajes de SOP Loom. Decisión de diseño central: **no depende de `qa_casos` para nada, ni siquiera por debajo.** Los valores de prueba se derivan de las restricciones reales declaradas en `aprendizaje.campos` — nunca de datos de negocio. Esto es intencional y contrario al principio de §6.1 a propósito: la Suite no valida negocio, valida requisitos técnicos, y para eso el dato controlado es el método correcto.

Se construyó en dos tandas: una v1 (Fases 0-4) y una segunda tanda de endurecimiento (Fases 5-9) después de ponerla a correr contra el entorno real y encontrar que el semáforo no era confiable. El detalle completo, fase por fase — qué se hizo, qué falta, criterios de aceptación — vive en **`docs/plan_suite_calidad.md`**; esta sección resume el estado resultante, no el camino.

**No hace falta que un aprendizaje esté aprobado para que la Suite lo corra** — alcanza con que esté cargado y compilado (tenga `definicion_ejecutable`). Los que sí están aprobados conservan la verificación de que la definición y la navegación no cambiaron desde la aprobación; los que no, solo necesitan tener `inspeccion_navegacion` para que el chequeo de deriva contra la pantalla real tenga contra qué comparar.

**Módulo backend** (`backend-node/src/modules/qa/suite/`):
- `qa-suite-payloads.ts` + `qa-suite-derivador.service.ts` (`QaSuiteDerivadorService.derivarEscenarios(aprendizajeId, pasos, campos, categoria)`) — motor determinístico. Estrategia "una variable a la vez": cada escenario varía un solo campo (con un candidato por restricción/payload) y completa el resto con un valor funcional seguro, para que el flujo pueda avanzar. Mismo aprendizaje + misma categoría → mismos escenarios, siempre.
  ...
  - `seguridad`: cada candidato trae `tipo: 'violacion_restriccion' | 'inyeccion'` — es lo que decide, en `run-qa-suite-seguridad.mjs`, qué oráculo aplicarle (ver más abajo). `violacion_restriccion` son valores que violan a propósito la restricción declarada (largo, rango, patrón, ventana de fechas); `inyeccion` son los payloads fijos (SQLi, NoSQLi, XSS, path traversal, template injection).
  ...
  - Para que un campo llegue con `restriccion` real (y no cause que la Suite derive genéricos como `"dato de prueba"` que la validación del propio formulario rechaza), el aprendizaje tiene que haberse compilado **después** de que `compilar()` en `qa-sop-loom.service.ts` empezara a persistirla (línea ~911) — un aprendizaje viejo necesita volver a guardarse desde SOP Loom para incorporarla; no hace falta migración de datos.
- **Espejo en JS plano** de ese motor en `backend-node/scripts/lib/qa-suite-derivador.mjs` — los scripts de Playwright no importan TypeScript compilado, son procesos Node autónomos (mismo patrón que el resto del proyecto). **Si se cambia el algoritmo en el `.service.ts`, hay que replicarlo a mano en el `.mjs`** — no hay una sola fuente de verdad, es una duplicación deliberada. Hay un test de paridad (`qa-suite-derivador.service.spec.ts`, corre el `.mjs` en un subproceso Node real porque Jest no sabe interpretar ESM vía `import()` dinámico) que falla si los dos se desincronizan.
- `qa-suite-runner.service.ts` — orquesta: gate de aprobación condicional (arriba), calcula el tiempo máximo de la corrida en función de la cantidad de escenarios derivados (no un valor fijo por modo — un aprendizaje con muchos campos necesita más tiempo), spawnea el script de la categoría con un directorio de salida y un id de ejecución propios (env vars `AUDITORIA_QA_SUITE_OUTPUT_DIR`/`AUDITORIA_QA_SUITE_EJECUCION`, para que dos corridas no se pisen evidencia), parsea su stdout/evidencia, arma hallazgos, **borra de `qa_casos` cualquier documento que la ejecución haya creado** (ver aislamiento de datos, abajo) y arma el informe consolidado.
  - `estado` de una ejecución y `estado_consolidado` de una corrida distinguen `rojo` (la app falló una prueba real — hay evidencia) de **`error`** (la Suite no llegó a correr la prueba — sin evidencia: se cayó la conexión, la pantalla cambió desde la aprobación, se agotó el tiempo máximo). El consolidado de una corrida prioriza el estado de sus ejecuciones por sobre la severidad de los hallazgos: una sola ejecución en `error` o `rojo` alcanza para que la corrida entera no sea `verde`, tenga o no hallazgos registrados.
- Scripts: `run-qa-suite-funcional.mjs`, `run-qa-suite-accesibilidad.mjs` (audita con **axe-core** — WCAG 2.0/2.1 A/AA — inyectado de cero para este módulo), y `run-qa-suite-seguridad.mjs`, el más elaborado:
  - **Sesión** — dos comprobaciones: la ruta objetivo no puede abrirse sin cookie (rebote a `/login`, es código de cliente); y los endpoints que el flujo usa para guardar tienen que rechazar (401/403) una petición directa sin cookie (la autorización real del servidor).
  - **Violación de restricción** — el paso de guardado se corre aparte del resto (`intentarGuardado()`, espera corta de 8s, no lanza si no llega respuesta: un valor bloqueado por la validación del formulario es el resultado esperado, no un error de la Suite). Si el guardado igual acepta un valor que viola la restricción declarada, hallazgo de severidad alta.
  - **Inyección** — solo tiene oráculo de efecto real para payloads con forma de XSS: guarda, reabre `/qa/casos` (donde el módulo muestra la descripción) y confirma que no dispara un `dialog` nativo (lo que un `alert()` real produciría). Los payloads de SQLi/NoSQLi/path-traversal/plantillas se mandan igual (para detectar si rompen el flujo) pero **sin oráculo todavía** — esta app no arma SQL ni filtros Mongo dinámicos a partir de ese valor, así que no hay nada real que afirmar. La Pantalla 4 declara este alcance parcial explícitamente en la propia card de "Seguridad" (frentes implementados vs. pendientes), no lo insinúa como cobertura completa.
  - Pendientes, documentados tanto en el script como en la UI: IDOR, manipulación de parámetros de negocio, exposición de datos sensibles, carga de archivos, cabeceras de transporte.
- Helper compartido `scripts/lib/qa-suite-comun.mjs`: login, conexión Mongo, `cargarAprendizaje()`, `validarNavegacionAprobada()` (reproduce el mismo hash de inventario de pantalla que `qa-pantalla-inspector.service.ts` — 8 campos por elemento, `testid/tag/tipo/rol/nombre/etiqueta/obligatorio/opciones`; una versión reducida de ese hash es lo que rompía esto en todas las corridas hasta que se corrigió), `ejecutarPaso()` (adaptado del ejecutor de pasos de SOP Loom), `mapearCampos()` (adapta `{nombre,...}` de `doc.campos` a la forma `{clave,...}` que espera el derivador), `activarAislamientoDatos()` y `idDesdeRespuesta()` (aislamiento de datos, abajo).
- **Aislamiento de datos** (Fase 7): el último paso de un flujo de alta guarda un caso real contra `/api/qa/casos`. `activarAislamientoDatos(context, ejecucionId)` intercepta ese POST con `context.route()` y le agrega `origen.suite_ejecucion_id` al cuerpo antes de que salga — no depende de que el frontend o el backend sepan nada de la Suite. Al cerrar la ejecución, `qa-suite-runner.service.ts` borra sin condición todo lo que quedó marcado con ese id (corra como corra la ejecución, incluida la que mató el timeout) y guarda cuántos documentos limpió en `casos_negocio_limpiados`. `idDesdeRespuesta()` lee el `id` real que el backend devuelve al guardar (no un id inventado) — se usa tanto para el aislamiento como para que `verificar_fila` (antes siempre `omitido`, porque dependía de un caso QA congelado que la Suite no usa) pueda confirmar que la fila nueva apareció en el listado.
- **Modo demo** abre una ventana real de Chromium — necesita un entorno gráfico (`DISPLAY`/`WAYLAND_DISPLAY` en Linux, sesión interactiva en macOS/Windows). Si no lo detecta (o corre en CI), degrada solo a headless en vez de que `chromium.launch()` reviente — con una advertencia en el log de la corrida.

**Colecciones propias** (todas nuevas, ninguna comparte schema con lo viejo):
- `qa_suite_ejecuciones` — una corrida de una categoría sobre un aprendizaje. `aprendizaje_id`, `categoria: 'funcional'|'seguridad'|'accesibilidad'`, `modo: 'rapido'|'demo'`, `estado: 'corriendo'|'verde'|'rojo'|'error'`, `casos_negocio_limpiados`.
...
- `qa_suite_corridas` — agrupa N ejecuciones (aprendizajes × categorías) de una corrida disparada junto. `estado_consolidado: 'corriendo'|'verde'|'amarillo'|'rojo'|'error'`. `informe.por_aprendizaje[id]` trae ficha, semáforo, tabla por categoría, hallazgos priorizados, evidencia, comparación contra la corrida anterior sobre el mismo aprendizaje.

**Frontend**: `QaPantalla4Component` (`frontend-angular/src/app/pages/qa-pantalla-4/`), servicio `qa-suite.service.ts`. Flujo de la UI: elegir aprendizajes + categorías + modo (la card de cada aprendizaje muestra su estado real — borrador/revisar/listo/aprobado — y sus firmas, no un "Aprobado" fijo) → botón "Correr suite" abre un **modal de vista previa** (`QaSuiteVistaPreviaDialogComponent`, mismo archivo) que llama a `POST /qa/suite/vista-previa` para mostrar exactamente qué valor se va a escribir en cada campo y por qué, **antes** de correr nada — el usuario confirma recién ahí. Después: matriz de resultados (aprendizajes × categorías, mismo estilo de estado-pill que `/qa/casos`, con `error` como color propio distinto de `rojo`) → informe por corrida.

**Gotcha ya encontrado con este módulo específico**: el entorno compartido hace que un aprendizaje se invalide (por deriva de pantalla) o desaparezca (`activo: false`) mientras se lo está usando para probar algo — pasó dos veces en la misma sesión con la misma pantalla, porque alguien más la estaba editando en vivo. Antes de asumir que una corrida en `error` es un bug del código de la Suite, confirmar que el aprendizaje usado sigue existiendo, activo, y sin deriva.
